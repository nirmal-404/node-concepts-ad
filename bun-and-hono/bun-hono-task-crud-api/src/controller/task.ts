...
export async function createTask (c: Context, db: Database) {
  ...
  if (!userId) {
    return c.json(
      { error: 'Unauthenticated! Tou need to log in to create tasks' },
      401
    )
  }
  if (userRole !== 'admin') {
    return c.json({ error: 'Unauthorized! Only admins can create a post' }, 403)
  }

  if (userId !== user_id) {
    return c.json({ error: 'Invalaid user with differrent userId' }, 403)
  }
  try {
    const result = db
      .query(
